...
Page({
  ...
  onShow: function () {
    //查询预约设置状态
    var that=this;
    var is_edit_set_yuyue=wx.getStorageSync('is_edit_set_yuyue');
    var yuyue_data=this.data.yuyue_data;
    console.log(yuyue_data)
    if(is_edit_set_yuyue){ 
      //读取上次保存的预约配置
      this.setData({
        is_pay:wx.getStorageSync('is_edit_yuyue_pay'),
        is_coupon:wx.getStorageSync('is_edit_yuyue_coupon'),
        max_stu:wx.getStorageSync('edit_max_stu'),
        // duration:wx.getStorageSync('duration'),
        before_time:wx.getStorageSync('edit_forbid_app_time'),
        end_time:wx.getStorageSync('edit_forbid_cancel_time'),
        clock_arr:JSON.parse(wx.getStorageSync('edit_time_set'))
      })
      ...
      //获取课程预约配置信息
    wx.request({ 
      url: app.globalData.url + 'index/YuYue/appointmentConfigInfo',
      data: {
      ...
      success(res){
        ...
        if(res.data){
          ...
          console.log('加载原来的数据')
          that.setData({
            is_pay:res.data.is_pay==1?true:false,
            is_coupon:res.data.is_coupon==1?true:false,
            before_time:res.data.forbid_app_time,
            end_time:res.data.forbid_cancel_time,
            max_stu:res.data.max_stu,
            clock_arr:JSON.parse(res.data.time_set)
          })
        }else{
            //按系统默认数据初始化
...
//保存函数
  save:function(){
    ...
    if(week.length==0){
      ...
      console.log(clock_arr)
      //将设置内容放在缓存
      wx.setStorageSync('is_edit_yuyue_pay', that.data.is_pay)
      wx.setStorageSync('is_edit_yuyue_coupon', that.data.is_coupon)
      wx.setStorageSync('edit_max_stu',parseInt(that.data.max_stu))
      // wx.setStorageSync('duration', that.data.array[that.data.index])
      wx.setStorageSync('edit_forbid_app_time', that.data.before_time)
      wx.setStorageSync('edit_forbid_cancel_time', that.data.end_time)
      wx.setStorageSync('edit_time_set',JSON.stringify(clock_arr))
      //设置预约设置状态  点击保存表示已更改
      wx.setStorageSync('is_edit_set_yuyue', true)
      setTimeout(function(){
        $Toast.hide();
        $Toast({
          content: '保存成功',
