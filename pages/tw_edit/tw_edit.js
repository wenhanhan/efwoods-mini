...
Page({
  ...
  onEditorReady() {
    ...
    var is_set_courDes=wx.getStorageSync('is_set_courDes');//是否保存过课程详情
    wx.createSelectorQuery().select('#editor').context(function (res) {
      that.editorCtx = res.context
      if(is_set_courDes){
        that.editorCtx.setContents({
          html:wx.getStorageSync('cour_des'),
          success:()=>{
            wx.hideLoading({
              complete: (res) => {},
            })
  ...
  save:function(){
    ...
    this.editorCtx.getContents({
      ...
      wx.setStorageSync('cour_des', res.html)
      wx.setStorageSync('is_set_courDes', true)
      setTimeout(function(){
        $Toast.hide();
        $Toast({
          content: '保存成功',
