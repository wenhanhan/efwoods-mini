...
//小标签设置
  ...
  onLoad: function (options) {
    ...
    query.select('#course_box').boundingClientRect(
      (rect) => {
        let top = rect.top;//筛选导航距离上边界的大小
        // this.setData({
        //   swiper_height: top - this.data.top_bar_height
        // })
        console.log('课程盒子高度' + top)
      }
    ).exec();
...
that.address(province, city, district, latitude, longitude, type,sex,start_time,end_time, tag, paixu,page)
  ...
  qqmapsdk.geocoder({
    ...
    success: function (res) {
      ...
      // var type = that.data.fenlei_index == -1 ? '' : that.data.fenlei_index;//课程类别
      var sex = '', hour = '', tag = '';
      var start_time = new_city ? '' : that.data.start_time;
      var end_time = new_city ? '' : that.data.end_time;
      var cate = that.data.cate;//选好的排序分类
      var shaixuan_arr = that.data.shaixuan_arr;//选好的选项
      for (var i = 0; i < cate.length; i++) {
        if (cate[i] == 'sex') {
      ...
      that.address(select_province, city, select_district, latitude, longitude, type, sex, start_time, end_time, tag, paixu,page)
    }
  }) 
}
    ...
    //课程轮播图
    wx.request({
      url: app.globalData.url + 'index/Lunbo/getCourseBanner',
      data: {},
      header: {
  ...
  //区域筛选课程函数
  address: function (province, city, district, latitude, longitude, type, sex, start_time, end_time, tag, paixu,page){
      var that=this;
      var address=province+city+district
      wx.request({
        url: app.globalData.url+'index/shaixuan/address',
        data:{
          address: address,
          latitude: latitude,
          longitude: longitude,
        ...
        success(res) { 
          ...
          if(page==1){
            that.setData({
              course:res.data[0],
              top_course:res.data[1]
            })
          }else{
            if(!that.data.noMoreData){
              that.setData({
                course:course.concat(res.data[0]),
                top_course:res.data[1]
              })
            }else{
              return;
...
//遮罩点击隐藏事件
  ...
  select_paixu:function(e){
    ...
    wx.setStorageSync('cour_paixu', paixu)
